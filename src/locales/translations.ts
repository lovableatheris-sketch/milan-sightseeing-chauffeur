export const translations = {
  pt: {
    header: {
      brand: "Nexus",
      inicio: "Início",
      chiSiamo: "Quem somos?",
      nostriVeicoli: "Nossos veículos",
      servicos: "Serviços",
      contato: "Contato",
      reserveAgora: "Reserve Agora",
    },
    common: {
      whatsappMessage: "Olá, venho do site da Nexus",
    },
    hero: {
      title: "A excelência passa despercebida. Ela é vivida.",
      titleHighlight: "",
      subtitle:
        "Nexus é a plataforma de mobilidade premium pensada para toda necessidade de transfer e eventos, disponível em diversas cidades.",
      cta: "Nexus Company",
      ctaContact: "Entre em Contato",
    },
    chiSiamo: {
      title: "Quem somos?",
      subtitle: "Profissionalismo, confiabilidade e conforto",
      description:
        "Nexus é o seu serviço NCC (Noleggio Con Conducente) de referência em Milão. Oferecemos transfers aeroportuários de e para Malpensa, Linate e Bergamo Orio al Serio com veículos Mercedes de luxo e motoristas profissionais multilíngues. Diferente dos táxis tradicionais, garantimos preço fixo comunicado antecipadamente, sem surpresas, sem suplementos.",
      descriptionExtra:
        "Operamos 24 horas por dia, 7 dias por semana com monitoramento de voos em tempo real. Nosso motorista te espera no aeroporto com placa nominal, ajuda com as bagagens e te acompanha até o veículo climatizado.",
    },
    experienceGallery: {
      title: "Nossas Experiências",
      description: "Descubra os destinos incríveis e a qualidade dos nossos serviços",
    },
    whyChoose: {
      subtitle: "MODELO OPERACIONAL DE EXCELÊNCIA",
      title: "O PADRÃO NEXITUS DE MOBILIDADE",
      description:
        "Nossa trajetória nasce de anos de dedicação ao lado de uma clientela d'élite. Transformamos essa experiência em um modelo operacional em que a perfeição é a norma e a reatividade antecipa cada necessidade.",
      feature1: "PERFEIÇÃO COMO NORMA",
      feature2: "REATIVIDADE & ANTECIPAÇÃO",
      feature3: "CLIENTELA D'ÉLITE & DISCRIÇÃO",
      cardTitle: "Modelo Operacional & Garantia de Qualidade NEXITUS",
      cardItem1Title: "✓ Precisão e Pontualidade Absoluta",
      cardItem1Desc: "Planejamento preventivo com monitoramento em tempo real para garantir zero margens de atraso.",
      cardItem2Title: "✓ Frota Mercedes-Benz d'Élite",
      cardItem2Desc: "Veículos higienizados, conforto executivo, conexão WiFi e isolamento acústico para total privacidade.",
      cardItem3Title: "✓ Motoristas Dedicados & Recepção VIP",
      cardItem3Desc: "Serviço Meet & Greet com placa nominal, auxílio com bagagens e tratamento discreto e personalizado.",
      cardItem4Title: "✓ Operação e Suporte B2B 24/7",
      cardItem4Desc: "Disponibilidade contínua para transfers aeroportuários, viagens de negócios e logísticas de eventos.",
    },
    nostriVeicoli: {
      title: "Nossos veículos",
      subtitle: "Frota Mercedes de luxo para o seu conforto",
      vehicle1Title: "Mercedes Benz v250",
      vehicle1Desc:
        "Sedã elegante e confortável, perfeito para transfers e viagens de negócios. Capacidade: até 3 passageiros + bagagens. Equipado com ar condicionado, bancos de couro e tecnologia de última geração.",
      vehicle2Title: "Mercedes Classe V",
      vehicle2Desc:
        "Van de luxo ideal para grupos e famílias. Capacidade: até 7 passageiros + bagagens. Espaço amplo, conforto superior e perfeito para passeios turísticos e transfers aeroportuários com várias pessoas.",
      calcPreventivo: "Calcular orçamento",
      passengers: "passageiros",
      luggage: "Bagagem",
    },
    services: {
      title: "Nossos Serviços",
      subtitle: "Serviços de noleggio con conducente para todas as suas necessidades de transporte executivo",
      touristTitle: "Tours Turísticos",
      touristDesc: "Lagos e destinos com motorista particular",
      disposalTitle: "Disposições Empresariais",
      disposalDesc: "Carro à disposição por hora - 2h, 4h ou 8h",
      whatsappTitle: "Reservar WhatsApp 24h",
      whatsappDesc: "Atendimento imediato via WhatsApp",
      transfersTitle: "Transfers Aeroportos",
      transfersDesc: "Malpensa, Linate, Bergamo porta-a-porta",
      longDistanceTitle: "Viagens Longas",
      longDistanceDesc: "Tarifas por km para qualquer destino",
    },
    touristTours: {
      title: "Tours Turísticos",
      subtitle: "Descubra os lagos e destinos mais belos da região com conforto e estilo",
      lagoComoTitle: "Lago di Como",
      lagoComoDesc: "Tour completo ao elegante Lago di Como",
      lagoMaggioreTitle: "Lago Maggiore",
      lagoMaggioreDesc: "Explore as ilhas e jardins do Lago Maggiore",
      sanSiroTitle: "San Siro Stadium",
      sanSiroDesc: "Visite o templo do futebol italiano",
      priceFrom: "A partir de",
      duration: "Duração",
      hours: "horas",
      bookNow: "Reservar Agora",
    },
    hourlyDisposal: {
      title: "Serviços de Noleggio con Conducente",
      subtitle: "Disposições Horárias",
      description: "Carro à disposição com motorista particular para suas necessidades empresariais ou pessoais",
      package2h: "Pacote 2 Horas",
      package4h: "Pacote 4 Horas",
      package8h: "Pacote 8 Horas",
      price: "Preço",
      features: "Incluído",
      professionalDriver: "Motorista profissional",
      luxuryCar: "Veículo de luxo Mercedes",
      flexibility: "Total flexibilidade de itinerário",
      assistance: "Assistência 24h",
      bookNow: "Solicitar Orçamento",
    },
    transfers: {
      title: "Transfers Aeroportos",
      subtitle: "Serviço porta-a-porta para todos os aeroportos de Milão",
      mainTitle: "Oferecemos um serviço de aluguel com motorista de e para o aeroporto de Malpensa em Milão.",
      chooseService: "Escolha nosso serviço e aproveite várias vantagens:",
      flightMonitoring: "planeje sua viagem com antecedência usando um dos veículos à sua disposição",
      comfortTravel: "viaje com total conforto em um carro de luxo",
      inclusivePrice:
        "conheça o preço inclusivo da sua viagem com antecedência: é fornecido ao solicitar um orçamento, para evitar surpresas desagradáveis e extras caros",
      doorToDoor:
        "chegue ao seu destino final rapidamente: um serviço porta-a-porta que economizará tempo e garantirá uma experiência excepcional",
      highlightTitle: "SEU TRANSFER DE E PARA MILÃO MALPENSA, SEM RISCO E SEM ESTRESSE",
      contactUs: "CONTATE-NOS",
      malpensa: "Aeroporto Malpensa",
      linate: "Aeroporto Linate",
      bergamo: "Bergamo/Orio al Serio",
      priceFrom: "A partir de",
      features: "Serviço Incluído",
      meetGreet: "Meet & Greet",
      luggageAssistance: "Assistência com bagagem",
      professionalDriver: "Motorista profissional",
      bookNow: "Reservar Transfer",
      pricesTitle: "Preços Transfer Aeroportos",
      malpensaMilano: "Malpensa – Milano",
      linateMilano: "Linate – Milano",
      orioMilano: "Orio al Serio – Milano",
    },
    longDistance: {
      title: "Viagens Longas - Tarifas por Km",
      subtitle: "Servizi di Noleggio con Conducente para qualquer destino na Itália",
      description: "Viaje com conforto para qualquer cidade italiana",
      perKm: "por km",
      examples: "Exemplos de Rotas",
      milanoBari: "Milano → Bari",
      milanoNapoli: "Milano → Napoli",
      milanoRoma: "Milano → Roma",
      milanoFirenze: "Milano → Firenze",
      anyDestination: "Milano → Qualquer Endereço",
      features: "Incluído no Serviço",
      professionalDriver: "Motorista experiente",
      luxuryCar: "Mercedes Classe E ou V",
      stops: "Paradas permitidas",
      assistance: "Assistência 24h",
      bookNow: "Solicitar Orçamento",
    },
    locations: {
      title: "Explore Milão com Exclusividade",
      subtitle: "Conheça os principais pontos turísticos da cidade com o conforto e a elegância que você merece",
      historicTitle: "San Siro",
      historicDesc:
        "Visite o lendário estádio San Siro, lar dos gigantes do futebol italiano Inter e Milan, um templo do esporte mundial",
      luxuryTitle: "Lago Maggiore",
      luxuryDesc:
        "Descubra as águas cristalinas e as ilhas encantadoras do Lago Maggiore, com seus jardins históricos e vistas deslumbrantes",
      cultureTitle: "Lago di Como",
      cultureDesc:
        "Explore o elegante Lago di Como, cercado por vilas luxuosas, montanhas majestosas e vilarejos pitorescos",
    },
    finalCta: {
      title: "A excelência tem um novo endereço.",
      titleHighlight: "Bem-vindo ao mundo NEXITUS.",
      subtitle: "Conecte-se com a evolução do serviço de mobilidade de altíssimo padrão.",
      subtitleExtra: "Perfeição operacional, discrição e reatividade a serviço da mobilidade executiva.",
      cta: "Solicitar Atendimento Executive",
    },
    tariffs: {
      title: "Calcular orçamento",
      description:
        "Confira nossas tarifas para destinos de montanha, lagos, praias e grandes cidades, partindo do Aeroporto di Malpensa.",
      viewAll: "Ver todas as tarifas",
      pageTitle: "Calcular orçamento",
      pageSubtitle: "Partindo do Aeroporto di Malpensa",
      distanceNote: "Todas as distâncias indicadas são ida e volta (A/R).",
      mountainsTitle: "MONTANHAS & LAGOS (+30%)",
      seaTitle: "MAR & GRANDES CIDADES",
      kmLabel: "Km (A/R):",
      priceLabel: "Preço:",
      ratesBox: {
        baseRate: "Tarifa quilométrica base:",
        mountainRate: "Lagos e Montanhas:",
        hourlyRate: "Disposições horárias:",
        olympicsRate: "Olimpíadas Milano-Cortina:",
      },
      bookNow: "Reserve Agora",
      milanoTransferTitle: "Milano - Transfer Simples",
      corporateTitle: "Corporate (com fatura)",
      corporateHours4: "4 horas:",
      corporateHours8: "8 horas:",
      capacityTitle: "Capacidade",
      capacityDesc: "Até 5–6 passageiros. Para confirmar o número de malas, entre em contato via WhatsApp.",
    },
    footer: {
      brand: "Nexus, the living experience",
      brandDesc:
        "Seu motorista particular de confiança em Milão. Transporte de luxo com excelência, conforto e pontualidade.",
      contactTitle: "Contato",
      whatsapp: "WhatsApp",
      socialTitle: "Redes Sociais",
      copyright: "© 2025 Nexus. Todos os direitos reservados.",
    },
    contact: {
      needInfo: "PRECISA DE MAIS INFORMAÇÕES?",
      title: "ENTRE EM CONTATO",
      welcomeText: "Estamos aqui para ajudar você a planejar sua viagem de forma simples e rápida.",
      form: {
        name: "Nome",
        email: "E-mail",
        phone: "Telefone",
        message: "Mensagem",
        privacyConsent:
          "Dou o consentimento para o processamento dos meus dados pessoais de acordo com os procedimentos e limitações estabelecidos na declaração de privacidade.",
        privacyLink: "Ler a Declaração de Privacidade",
        send: "Enviar",
        namePlaceholder: "Seu nome",
        emailPlaceholder: "seu@email.com",
        messagePlaceholder: "Como podemos ajudar?",
      },
      info: {
        phone: "Telefone",
        phoneNumber: "+39 389 143 0907",
        whatsapp: "WhatsApp",
        whatsappNumber: "+39 389 143 0907",
        email: "E-mail",
        emailAddress: "thomastagliatti2022@gmail.com",
        office: "Sede Legal",
        officeAddress: "Milano, Itália",
      },
      success: "Mensagem enviada com sucesso!",
      error: "Erro ao enviar mensagem. Tente novamente.",
    },
    milano: {
      navLabel: "Nexus Company",
      heroTitle: "MILANO",
      heroSubtitle: "Premium Mobility",
      heroTagline: "by Nexus",
      heroDescription: "Divisão corporativa dedicada a clientes B2B. Mobilidade executiva premium com padrões internacionais de excelência.",
      manifestoTitle: "Manifesto Operativo",
      manifestoText: "MILANO representa a evolução natural da excelência Nexus para o mercado corporativo. Nossa abordagem operativa combina experiência consolidada com flexibilidade total, garantindo soluções de mobilidade sob medida para cada necessidade empresarial.",
      standardsTitle: "Standard di Servizio",
      standard1Title: "Pontualidade Absoluta",
      standard1Desc: "Monitoramento de voos em tempo real. Zero tolerância a atrasos.",
      standard2Title: "Frota Premium",
      standard2Desc: "Mercedes exclusivos, sanificados e com manutenção certificada.",
      standard3Title: "Motoristas Qualificados",
      standard3Desc: "Profissionais multilíngues com treinamento corporativo.",
      standard4Title: "Faturamento Empresarial",
      standard4Desc: "Processo simplificado com fattura elettronica e relatórios detalhados.",
      flexibilityTitle: "Flessibilità Operativa",
      flexibilityText: "Adaptamos nossos serviços às exigências específicas de cada cliente corporate. De disposições horárias a contratos de longo prazo, oferecemos soluções personalizadas com condições dedicadas.",
      ctaContact: "Contato B2B",
      ctaWhatsApp: "WhatsApp Business",
      ctaDiscover: "Descobrir a Excelência",
      tariffsTitle: "Prontuario Tariffe Corporate",
      tariffsSubtitle: "Listino Pubblico e Ufficiale",
      tariffsDescription: "Prezzi chiari e trasparenti per servizi corporate. Tutte le tariffe sono IVA esclusa.",
      hourlyTitle: "Disposições Orarie",
      hours4: "4 horas",
      hours8: "8 horas",
      hours4Price: "€360",
      hours8Price: "€680",
      transfersTitle: "Transfer Aeroportuali",
      malpensaPrice: "€170",
      linatePrice: "€100",
      bergamoPrice: "€180",
      kmTitle: "Tariffa Chilometrica",
      kmBase: "€1,50/km",
      kmMountain: "€1,95/km (+30% laghi/montagna)",
      notes: "* Tariffe per Mercedes Classe V (fino a 6 pax). Per Classe S ou veículos especiais, richiedere preventivo dedicato.",
      invoiceNote: "Fatturazione elettronica disponibile. Condições especiais para contratos anuais.",
    },
    aboutNexitus: {
      tagline: "SOBRE A NEXITUS",
      title: "NEXITUS",
      subtitle: "Executive Mobility & Chauffeur Services",
      description: "A NEXITUS é referência em mobilidade executiva de alto padrão no Norte da Itália e Europa. Especializados em transfers aeroportuários para Malpensa, Linate e Orio al Serio, serviços à disposição horária e logística completa para eventos corporativos e viagens de negócios, oferecemos uma experiência onde a excelência é o único padrão aceito.",
      pillar1Title: "Frota Mercedes-Benz Exclusiva",
      pillar1Desc: "Veículos novos, impecavelmente limpos e higienizados após cada viagem. Conforto e sofisticação em cada detalhe.",
      pillar2Title: "Motoristas Profissionais Multilíngues",
      pillar2Desc: "Pontualidade absoluta, discrição total e recepção com placa nominal no desembarque. Profissionais treinados para o padrão corporativo.",
      pillar3Title: "Transparência e Flexibilidade B2B",
      pillar3Desc: "Preços fixos acordados previamente, faturamento corporativo eletrônico e atendimento dedicado 24 horas por dia, 7 dias por semana.",
      ctaServices: "Conhecer Nossos Serviços",
      ctaContact: "Atendimento B2B",
    },
  },
  en: {
    header: {
      brand: "Nexus, the living experience",
      inicio: "Home",
      chiSiamo: "About us",
      nostriVeicoli: "Our vehicles",
      servicos: "Services",
      contato: "Contact",
      reserveAgora: "Book Now",
    },
    common: {
      whatsappMessage: "Hi, I'm coming from the Nexu site"
    },
    hero: {
      title: "Excellence goes unnoticed. It is lived.",
      titleHighlight: "",
      subtitle:
        "NEXUS is the premium mobility platform designed for every transfer and event need, available in several cities.",
      cta: "NEXUS COMPANY",
      ctaContact: "Contact Us",
    },
    chiSiamo: {
      title: "About us",
      subtitle: "Professionalism, reliability and comfort",
      description:
        "Nexus is your reference NCC (Noleggio Con Conducente) service in Milan. We offer airport transfers to and from Malpensa, Linate and Bergamo Orio al Serio with luxury Mercedes vehicles and multilingual professional drivers. Unlike traditional taxis, we guarantee a fixed price communicated in advance, no surprises, no supplements.",
      descriptionExtra:
        "We operate 24 hours a day, 7 days a week with real-time flight monitoring. Our driver waits for you at the airport with a name sign, helps with luggage and accompanies you to the air-conditioned vehicle.",
    },
    experienceGallery: {
      title: "Our Experiences",
      description: "Discover incredible destinations and the quality of our services",
    },
    whyChoose: {
      subtitle: "EXECUTIVE OPERATIONAL MODEL",
      title: "THE NEXITUS STANDARD OF MOBILITY",
      description:
        "Our story stems from years of dedication alongside an elite clientele. We turned this expertise into an operational framework where perfection is standard and proactive responsiveness anticipates every need.",
      feature1: "PERFECTION AS STANDARD",
      feature2: "PROACTIVE RESPONSIVENESS",
      feature3: "ELITE DISCRETION & EXPERTISE",
      cardTitle: "NEXITUS Operational Excellence & Quality Commitment",
      cardItem1Title: "✓ Absolute Precision & Punctuality",
      cardItem1Desc: "Preventive planning and real-time flight tracking ensuring zero tolerance for delays.",
      cardItem2Title: "✓ Elite Mercedes-Benz Fleet",
      cardItem2Desc: "Sanitised vehicles equipped with executive comfort, onboard Wi-Fi, and acoustic privacy.",
      cardItem3Title: "✓ Dedicated Chauffeurs & VIP Welcome",
      cardItem3Desc: "Personalised Meet & Greet service, luggage assistance, and discreet corporate protocol.",
      cardItem4Title: "✓ 24/7 B2B Operational Support",
      cardItem4Desc: "Round-the-clock availability for airport transfers, hourly disposal, and event logistics.",
    },
    nostriVeicoli: {
      title: "Our vehicles",
      subtitle: "Luxury Mercedes fleet for your comfort",
      vehicle1Title: "Mercedes Benz v250",
      vehicle1Desc:
        "Elegant and comfortable sedan, perfect for transfers and business trips. Capacity: up to 3 passengers + luggage. Equipped with air conditioning, leather seats and latest technology.",
      vehicle2Title: "Mercedes V-Class",
      vehicle2Desc:
        "Luxury van ideal for groups and families. Capacity: up to 7 passengers + luggage. Spacious interior, superior comfort and perfect for tourist tours and airport transfers with multiple people.",
      calcPreventivo: "Calculate quote",
      passengers: "passengers",
      luggage: "Luggage",
    },
    services: {
      title: "Our Services",
      subtitle: "Noleggio con conducente services for all your executive transportation needs",
      touristTitle: "Tourist Tours",
      touristDesc: "Lakes and destinations with private driver",
      disposalTitle: "Corporate Disposal",
      disposalDesc: "Car at disposal per hour - 2h, 4h or 8h",
      whatsappTitle: "Book WhatsApp 24h",
      whatsappDesc: "Immediate service via WhatsApp",
      transfersTitle: "Airport Transfers",
      transfersDesc: "Malpensa, Linate, Bergamo door-to-door",
      longDistanceTitle: "Long Distance",
      longDistanceDesc: "Per km rates to any destination",
    },
    touristTours: {
      title: "Tourist Tours",
      subtitle: "Discover the most beautiful lakes and destinations in the region with comfort and style",
      lagoComoTitle: "Lake Como",
      lagoComoDesc: "Complete tour to the elegant Lake Como",
      lagoMaggioreTitle: "Lake Maggiore",
      lagoMaggioreDesc: "Explore the islands and gardens of Lake Maggiore",
      sanSiroTitle: "San Siro Stadium",
      sanSiroDesc: "Visit the temple of Italian football",
      priceFrom: "From",
      duration: "Duration",
      hours: "hours",
      bookNow: "Book Now",
    },
    hourlyDisposal: {
      title: "Noleggio con Conducente Services",
      subtitle: "Hourly Disposal",
      description: "Car at disposal with private driver for your business or personal needs",
      package2h: "2 Hours Package",
      package4h: "4 Hours Package",
      package8h: "8 Hours Package",
      price: "Price",
      features: "Included",
      professionalDriver: "Professional driver",
      luxuryCar: "Luxury Mercedes vehicle",
      flexibility: "Full itinerary flexibility",
      assistance: "24h assistance",
      bookNow: "Request Quote",
    },
    transfers: {
      title: "Airport Transfers",
      subtitle: "Door-to-door service to all Milan airports",
      mainTitle: "We provide a Hire with driver service to and from Milan Malpensa airport.",
      chooseService: "Choose our service and enjoy various advantages:",
      flightMonitoring: "plan your journey in advance using one of the vehicles at your disposal",
      comfortTravel: "travel in complete comfort in a luxury car",
      inclusivePrice:
        "know the inclusive price of your trip in advance: it's provided when you get a quote, so you avoid unpleasant surprises and expensive extras",
      doorToDoor:
        "reach your final destination quickly: a door to door service that will save you time and guarantee you an outstanding experience",
      highlightTitle: "YOUR TRANSFER TO AND FROM MILAN MALPENSA, RISK AND STRESS FREE",
      contactUs: "CONTACT US",
      malpensa: "Malpensa Airport",
      linate: "Linate Airport",
      bergamo: "Bergamo/Orio al Serio",
      priceFrom: "From",
      features: "Service Included",
      meetGreet: "Meet & Greet",
      luggageAssistance: "Luggage assistance",
      professionalDriver: "Professional driver",
      bookNow: "Book Transfer",
      pricesTitle: "Airport Transfer Prices",
      malpensaMilano: "Malpensa – Milan",
      linateMilano: "Linate – Milan",
      orioMilano: "Orio al Serio – Milan",
    },
    longDistance: {
      title: "Long Distance - Per Km Rates",
      subtitle: "Noleggio con Conducente services to any destination in Italy",
      description: "Travel in comfort to any Italian city",
      perKm: "per km",
      examples: "Route Examples",
      milanoBari: "Milano → Bari",
      milanoNapoli: "Milano → Napoli",
      milanoRoma: "Milano → Roma",
      milanoFirenze: "Milano → Firenze",
      anyDestination: "Milano → Any Address",
      features: "Included in Service",
      professionalDriver: "Experienced driver",
      luxuryCar: "Mercedes E or V Class",
      stops: "Stops allowed",
      assistance: "24h assistance",
      bookNow: "Request Quote",
    },
    locations: {
      title: "Explore Milan with Exclusivity",
      subtitle: "Discover the city's main tourist attractions with the comfort and elegance you deserve",
      historicTitle: "San Siro",
      historicDesc:
        "Visit the legendary San Siro stadium, home to Italian football giants Inter and Milan, a temple of world sport",
      luxuryTitle: "Lago Maggiore",
      luxuryDesc:
        "Discover the crystal-clear waters and enchanting islands of Lake Maggiore, with its historic gardens and breathtaking views",
      cultureTitle: "Lago di Como",
      cultureDesc:
        "Explore the elegant Lake Como, surrounded by luxury villas, majestic mountains and picturesque villages",
    },
    finalCta: {
      title: "Excellence has a new address.",
      titleHighlight: "Welcome to the world of NEXITUS.",
      subtitle: "Connect with our platform to experience the evolution of high-end mobility.",
      subtitleExtra: "Operational perfection, privacy, and proactive service for elite travel.",
      cta: "Contact Executive Desk",
    },
    tariffs: {
      title: "Calculate quote",
      description:
        "Check our rates for mountain, lake, beach and major city destinations, departing from Malpensa Airport.",
      viewAll: "View all rates",
      pageTitle: "Calculate quote",
      pageSubtitle: "Departing from Malpensa Airport",
      distanceNote: "All distances indicated are round trip (A/R).",
      mountainsTitle: "MOUNTAINS & LAKES (+30%)",
      seaTitle: "SEA & MAJOR CITIES",
      kmLabel: "Km (A/R):",
      priceLabel: "Price:",
      ratesBox: {
        baseRate: "Base per-kilometer rate:",
        mountainRate: "Lakes and Mountains:",
        hourlyRate: "Hourly disposal:",
        olympicsRate: "Milano-Cortina Olympics:",
      },
      bookNow: "Book Now",
      milanoTransferTitle: "Milan - Simple Transfer",
      corporateTitle: "Corporate (with invoice)",
      corporateHours4: "4 hours:",
      corporateHours8: "8 hours:",
      capacityTitle: "Capacity",
      capacityDesc: "Up to 5–6 passengers. To confirm luggage quantity, contact us via WhatsApp.",
    },
    footer: {
      brand: "Nexus, the living experience",
      brandDesc:
        "Your trusted private driver in Milan. Luxury transportation with excellence, comfort and punctuality.",
      contactTitle: "Contact",
      whatsapp: "WhatsApp",
      socialTitle: "Social Media",
      copyright: "© 2025 Nexus. All rights reserved.",
    },
    contact: {
      needInfo: "NEED MORE INFORMATION?",
      title: "CONTACT US",
      welcomeText: "We're here to help you plan your trip quickly and easily.",
      form: {
        name: "Name",
        email: "E-mail",
        phone: "Phone",
        message: "Message",
        privacyConsent:
          "I consent to the processing of my personal data in accordance with the procedures and limitations set out in the privacy statement.",
        privacyLink: "Read the Privacy Statement",
        send: "Send",
        namePlaceholder: "Your name",
        emailPlaceholder: "your@email.com",
        messagePlaceholder: "How can we help?",
      },
      info: {
        phone: "Phone",
        phoneNumber: "+39 389 143 0907",
        whatsapp: "WhatsApp",
        whatsappNumber: "+39 389 143 0907",
        email: "E-mail",
        emailAddress: "thomastagliatti2022@gmail.com",
        office: "Registered Office",
        officeAddress: "Milano, Italy",
      },
      success: "Message sent successfully!",
      error: "Error sending message. Please try again.",
    },
    milano: {
      navLabel: "Nexus Company",
      heroTitle: "MILANO",
      heroSubtitle: "Premium Mobility",
      heroTagline: "by Nexus",
      heroDescription: "Corporate division dedicated to B2B clients. Premium executive mobility with international standards of excellence.",
      manifestoTitle: "Operational Manifesto",
      manifestoText: "MILANO represents the natural evolution of Nexus excellence for the corporate market. Our operational approach combines consolidated experience with total flexibility, guaranteeing tailored mobility solutions for every business need.",
      standardsTitle: "Service Standards",
      standard1Title: "Absolute Punctuality",
      standard1Desc: "Real-time flight monitoring. Zero tolerance for delays.",
      standard2Title: "Premium Fleet",
      standard2Desc: "Exclusive Mercedes, sanitized and with certified maintenance.",
      standard3Title: "Qualified Chauffeurs",
      standard3Desc: "Multilingual professionals with corporate training.",
      standard4Title: "Corporate Invoicing",
      standard4Desc: "Simplified process with electronic invoice and detailed reports.",
      flexibilityTitle: "Operational Flexibility",
      flexibilityText: "We adapt our services to the specific requirements of each corporate client. From hourly disposals to long-term contracts, we offer customized solutions with dedicated conditions.",
      ctaContact: "B2B Contact",
      ctaWhatsApp: "WhatsApp Business",
      ctaDiscover: "Discover Excellence",
      tariffsTitle: "Corporate Tariff Guide",
      tariffsSubtitle: "Official Public Price List",
      tariffsDescription: "Clear and transparent prices for corporate services. All rates exclude VAT.",
      hourlyTitle: "Hourly Disposal",
      hours4: "4 hours",
      hours8: "8 hours",
      hours4Price: "€360",
      hours8Price: "€680",
      transfersTitle: "Airport Transfers",
      malpensaPrice: "€170",
      linatePrice: "€100",
      bergamoPrice: "€180",
      kmTitle: "Per Km Rate",
      kmBase: "€1.50/km",
      kmMountain: "€1.95/km (+30% lakes/mountains)",
      notes: "* Rates for Mercedes V-Class (up to 6 pax). For S-Class or special vehicles, request dedicated quote.",
      invoiceNote: "Electronic invoicing available. Special conditions for annual contracts.",
    },
    aboutNexitus: {
      tagline: "ABOUT NEXITUS",
      title: "NEXITUS",
      subtitle: "Executive Mobility & Chauffeur Services",
      description: "NEXITUS is the benchmark for premium executive mobility in Northern Italy and across Europe. Specialising in airport transfers to Malpensa, Linate and Orio al Serio, hourly chauffeur services, and end-to-end logistics for corporate events and business travel, we deliver an experience where excellence is the only accepted standard.",
      pillar1Title: "Exclusive Mercedes-Benz Fleet",
      pillar1Desc: "Brand-new vehicles, impeccably clean and sanitised after every journey. Comfort and sophistication in every detail.",
      pillar2Title: "Multilingual Professional Chauffeurs",
      pillar2Desc: "Absolute punctuality, complete discretion and meet-and-greet with a personal name board at arrivals. Professionals trained to corporate standards.",
      pillar3Title: "B2B Transparency & Flexibility",
      pillar3Desc: "Fixed prices agreed in advance, electronic corporate invoicing and dedicated support available 24 hours a day, 7 days a week.",
      ctaServices: "Explore Our Services",
      ctaContact: "B2B Enquiries",
    },
  },
  it: {
    header: {
      brand: "Nexus",
      inicio: "Home",
      chiSiamo: "Chi siamo?",
      nostriVeicoli: "I nostri veicoli",
      servicos: "Servizi",
      contato: "Contatto",
      reserveAgora: "Prenota Ora",
    },
    common: {
      whatsappMessage: "Salve, vengo dal sito Nexus",
    },
    hero: {
      title: "L'eccellenza non si nota. Si vive.",
      titleHighlight: "",
      subtitle:
        "Nexus è la piattaforma di mobilità premium pensata per ogni esigenza di transfer ed eventi, disponibile in diverse città.",
      cta: "NEXUS",
      ctaContact: "Contattaci",
    },
    chiSiamo: {
      title: "NCC Milano - Il Tuo Autista Privato di Fiducia",
      subtitle: "Noleggio con Conducente a Milano dal 2018",
      description:
        "Nexus è il tuo servizio NCC (Noleggio Con Conducente) di riferimento a Milano. Offriamo transfer aeroportuali da e per Malpensa, Linate e Bergamo Orio al Serio con veicoli Mercedes di lusso e autisti professionisti multilingue. A differenza dei taxi tradizionali, garantiamo prezzo fisso comunicato in anticipo, nessuna sorpresa, nessun supplemento. Siamo specializzati in transfer business, tour turistici ai laghi (Como, Maggiore, Garda) e servizi VIP per eventi aziendali. La nostra missione: comfort, puntualità e sicurezza in ogni viaggio.",
      descriptionExtra:
        "Operiamo 24 ore su 24, 7 giorni su 7 con monitoraggio voli in tempo reale. Il nostro autista ti aspetta in aeroporto con cartello nominativo, ti aiuta con i bagagli e ti accompagna al veicolo climatizzato. Serviamo Milano centro, hinterland milanese e tutta la Lombardia.",
    },
    experienceGallery: {
      title: "Le Nostre Esperienze",
      description: "Scopri destinazioni incredibili e la qualità dei nostri servizi",
    },
    whyChoose: {
      subtitle: "MODELLO OPERATIVO D'ECCELLENZA",
      title: "IL PARADIGMA NEXITUS",
      description:
        "La nostra storia non nasce dal caso, ma da anni di dedizione al fianco di una clientela d'élite. Abbiamo trasformato questa esperienza in un modello operativo dove la perfezione è la norma e la reattività anticipa ogni esigenza.",
      feature1: "PERFEZIONE COME NORMA",
      feature2: "REATTIVITÀ E ANTECIPAZIONE",
      feature3: "CLIENTELA D'ÉLITE & DISCREZIONE",
      cardTitle: "Modello Operativo & Garanzia di Qualità NEXITUS",
      cardItem1Title: "✓ Precisione e Puntualità Assoluta",
      cardItem1Desc: "Pianificazione preventiva e monitoraggio in tempo reale per garantire zero margini di ritardo.",
      cardItem2Title: "✓ Flotta Mercedes-Benz d'Élite",
      cardItem2Desc: "Veicoli sanificati, dotati di comfort executive, Wi-Fi di bordo e massimo isolamento acustico.",
      cardItem3Title: "✓ Autisti Dedicati & Accoglienza VIP",
      cardItem3Desc: "Servizio Meet & Greet personalizzato con cartello nominativo, assistenza bagagli e massima discrezione.",
      cardItem4Title: "✓ Operatività B2B e Supporto 24/7",
      cardItem4Desc: "Assistenza continua per trasferimenti aeroportuali, disposizioni orarie ed esigenze business.",
    },
    nostriVeicoli: {
      title: "Mercedes NCC Milano - La Nostra Flotta",
      subtitle: "Veicoli di lusso per transfer Malpensa e tour turistici",
      vehicle1Title: "Mercedes Benz V250 - Van di Lusso",
      vehicle1Desc:
        "Il nostro Mercedes-Benz V250 è il veicolo ideale per transfer aeroportuali Malpensa-Milano e tour turistici ai laghi. Capacità: fino a 6 passeggeri + 6 bagagli grandi. Dotato di sedili in pelle premium, climatizzazione tri-zona, WiFi gratuito a bordo e prese USB per ogni passeggero. Vetri oscurati per la massima privacy, sospensioni pneumatiche per un viaggio silenzioso e confortevole. Perfetto per famiglie, gruppi business e servizi VIP. Sanificato dopo ogni corsa con prodotti certificati.",
      vehicle2Title: "Mercedes Classe V",
      vehicle2Desc:
        "Van di lusso ideale per gruppi e famiglie. Capacità: fino a 7 passeggeri + bagagli. Spazio ampio, comfort superiore e perfetto per tour turistici e transfer aeroportuali con più persone.",
      calcPreventivo: "Calcola il preventivo",
      passengers: "passeggeri",
      luggage: "Bagagli",
    },
    services: {
      title: "I Nostri Servizi",
      subtitle: "Servizi di noleggio con conducente per tutte le vostre esigenze di trasporto esecutivo",
      touristTitle: "Pacchetti Turistici",
      touristDesc: "Laghi e destinazioni con autista privato",
      disposalTitle: "Disposizioni Aziendali",
      disposalDesc: "Auto a disposizione oraria - 2h, 4h o 8h",
      whatsappTitle: "Prenotare WhatsApp 24h",
      whatsappDesc: "Servizio immediato via WhatsApp",
      transfersTitle: "Transfer Aeroporti",
      transfersDesc: "Malpensa, Linate, Bergamo porta a porta",
      longDistanceTitle: "Viaggi Lunghi",
      longDistanceDesc: "Tariffe al km per qualsiasi destinazione",
    },
    touristTours: {
      title: "Pacchetti Turistici",
      subtitle: "Scopri i laghi e le destinazioni più belle della regione con comfort e stile",
      lagoComoTitle: "Lago di Como",
      lagoComoDesc: "Tour completo all'elegante Lago di Como",
      lagoMaggioreTitle: "Lago Maggiore",
      lagoMaggioreDesc: "Esplora le isole e i giardini del Lago Maggiore",
      sanSiroTitle: "Stadio San Siro",
      sanSiroDesc: "Visita il tempio del calcio italiano",
      priceFrom: "A partire da",
      duration: "Durata",
      hours: "ore",
      bookNow: "Prenota Ora",
    },
    hourlyDisposal: {
      title: "Servizi di Noleggio con Conducente",
      subtitle: "Disposizioni Orarie",
      description: "Auto a disposizione con autista privato per le vostre esigenze aziendali o personali",
      package2h: "Pacchetto 2 Ore",
      package4h: "Pacchetto 4 Ore",
      package8h: "Pacchetto 8 Ore",
      price: "Prezzo",
      features: "Incluso",
      professionalDriver: "Autista professionale",
      luxuryCar: "Veicolo di lusso Mercedes",
      flexibility: "Totale flessibilità di itinerario",
      assistance: "Assistenza 24h",
      bookNow: "Richiedi Preventivo",
    },
    transfers: {
      title: "Transfer Aeroporti",
      subtitle: "Servizio porta a porta per tutti gli aeroporti di Milano",
      mainTitle: "Offriamo un servizio Noleggio con Conducente da e per l'aeroporto di Milano Malpensa.",
      chooseService: "Scegli il nostro servizio e goditi vari vantaggi:",
      flightMonitoring: "pianifica il tuo viaggio in anticipo utilizzando uno dei veicoli a tua disposizione",
      comfortTravel: "viaggia in completo comfort in un'auto di lusso",
      inclusivePrice:
        "conosci il prezzo inclusivo del tuo viaggio in anticipo: viene fornito quando richiedi un preventivo, così eviti spiacevoli sorprese ed extra costosi",
      doorToDoor:
        "raggiungi rapidamente la tua destinazione finale: un servizio porta a porta che ti farà risparmiare tempo e ti garantirà un'esperienza eccezionale",
      highlightTitle: "IL TUO TRANSFER DA E PER MILANO MALPENSA, SENZA RISCHI E STRESS",
      contactUs: "CONTATTACI",
      malpensa: "Aeroporto Malpensa",
      linate: "Aeroporto Linate",
      bergamo: "Bergamo/Orio al Serio",
      priceFrom: "A partire da",
      features: "Servizio Incluso",
      meetGreet: "Meet & Greet",
      luggageAssistance: "Assistenza bagagli",
      professionalDriver: "Autista professionale",
      bookNow: "Prenota Transfer",
      pricesTitle: "Prezzi Transfer Aeroporti",
      malpensaMilano: "Malpensa – Milano",
      linateMilano: "Linate – Milano",
      orioMilano: "Orio al Serio – Milano",
    },
    longDistance: {
      title: "Viaggi Lunghi - Tariffe al Km",
      subtitle: "Servizi di Noleggio con Conducente per qualsiasi destinazione in Italia",
      description: "Viaggia con comfort verso qualsiasi città italiana",
      perKm: "al km",
      examples: "Esempi di Rotte",
      milanoBari: "Milano → Bari",
      milanoNapoli: "Milano → Napoli",
      milanoRoma: "Milano → Roma",
      milanoFirenze: "Milano → Firenze",
      anyDestination: "Milano → Qualsiasi Indirizzo",
      features: "Incluso nel Servizio",
      professionalDriver: "Autista esperto",
      luxuryCar: "Mercedes Classe E o V",
      stops: "Soste consentite",
      assistance: "Assistenza 24h",
      bookNow: "Richiedi Preventivo",
    },
    locations: {
      title: "Esplora Milano con Esclusività",
      subtitle: "Scopri le principali attrazioni turistiche della città con il comfort e l'eleganza che meriti",
      historicTitle: "San Siro",
      historicDesc:
        "Visita il leggendario stadio San Siro, casa dei giganti del calcio italiano Inter e Milan, un tempio dello sport mondiale",
      luxuryTitle: "Lago Maggiore",
      luxuryDesc:
        "Scopri le acque cristalline e le isole incantevoli del Lago Maggiore, con i suoi giardini storici e viste mozzafiato",
      cultureTitle: "Lago di Como",
      cultureDesc:
        "Esplora l'elegante Lago di Como, circondato da ville lussuose, montagne maestose e villaggi pittoreschi",
    },
    finalCta: {
      title: "L'eccellenza ha un nuovo indirizzo.",
      titleHighlight: "Benvenuti nel mondo NEXITUS.",
      subtitle: "Connettiti alla nostra pagina per seguire l'evoluzione del servizio d'alta gamma.",
      subtitleExtra: "Perfezione operativa, riservatezza e massima reattività al servizio della mobilità d'élite.",
      cta: "Richiedi Servizio Executive",
    },
    tariffs: {
      title: "Calcola il Preventivo",
      description:
        "Consulta le nostre tariffe per destinazioni di montagna, laghi, mare e grandi città, partendo dall'Aeroporto di Malpensa.",
      viewAll: "Visualizza tutte le tariffe",
      pageTitle: "Tariffe di Viaggio",
      pageSubtitle: "Partendo dall'Aeroporto di Malpensa",
      distanceNote: "Tutte le distanze indicate sono andata e ritorno (A/R).",
      mountainsTitle: "MONTAGNE & LAGHI (+30%)",
      seaTitle: "MARE & GRANDI CITTÀ",
      kmLabel: "Km (A/R):",
      priceLabel: "Prezzo:",
      ratesBox: {
        baseRate: "Tariffa chilometrica base:",
        mountainRate: "Laghi e Montagne:",
        hourlyRate: "Disposizioni orarie:",
        olympicsRate: "Olimpiadi Milano-Cortina:",
      },
      bookNow: "Prenota ora",
      milanoTransferTitle: "Milano - Transfer Semplice",
      corporateTitle: "Corporate (con fattura)",
      corporateHours4: "4 ore:",
      corporateHours8: "8 ore:",
      capacityTitle: "Capacità",
      capacityDesc: "Fino a 5–6 passeggeri. Per confermare il numero di valigie, contattaci via WhatsApp.",
    },
    footer: {
      brand: "Nexus, the living experience",
      brandDesc: "La tua auto privata di fiducia a Milano. Trasporto di lusso con eccellenza, comfort e puntualità.",
      contactTitle: "Contatto",
      whatsapp: "WhatsApp",
      socialTitle: "Social Media",
      copyright: "© 2025 Nexus. Tutti i diritti riservati.",
    },
    contact: {
      needInfo: "PRENOTA IL TUO TRANSFER MALPENSA",
      title: "Contatta Nexus - NCC Milano",
      welcomeText: "Prenota il tuo transfer aeroporto Malpensa-Milano in pochi minuti. Risposta immediata su WhatsApp 24 ore su 24. Preventivo gratuito e senza impegno per qualsiasi destinazione.",
      form: {
        name: "Nome",
        email: "E-mail",
        phone: "Telefono",
        message: "Messaggio",
        privacyConsent:
          "Acconsento al trattamento dei miei dati personali secondo le modalità e i limiti stabiliti nell'informativa sulla privacy.",
        privacyLink: "Leggi l'Informativa sulla Privacy",
        send: "Invia",
        namePlaceholder: "Il tuo nome",
        emailPlaceholder: "tua@email.com",
        messagePlaceholder: "Come possiamo aiutarti?",
      },
      info: {
        phone: "Telefono",
        phoneNumber: "+39 389 143 0907",
        whatsapp: "WhatsApp",
        whatsappNumber: "+39 389 143 0907",
        email: "E-mail",
        emailAddress: "thomastagliatti2022@gmail.com",
        office: "Sede Legale",
        officeAddress: "Milano, Italia",
      },
      success: "Messaggio inviato con successo!",
      error: "Errore nell'invio del messaggio. Riprova.",
    },
    milano: {
      navLabel: "Nexus Company",
      heroTitle: "NEXUS",
      heroSubtitle: "Premium Mobility",
      heroTagline: "by Nexus",
      heroDescription: "Divisione corporate dedicata a clienti B2B. Mobilità esecutiva premium con standard internazionali di eccellenza.",
      manifestoTitle: "Manifesto Operativo",
      manifestoText: "MILANO rappresenta l'evoluzione naturale dell'eccellenza Nexus per il mercato corporate. Il nostro approccio operativo combina esperienza consolidata con flessibilità totale, garantendo soluzioni di mobilità su misura per ogni esigenza aziendale.",
      standardsTitle: "Standard di Servizio",
      standard1Title: "Puntualità Assoluta",
      standard1Desc: "Monitoraggio voli in tempo reale. Zero tolleranza ai ritardi.",
      standard2Title: "Flotta Premium",
      standard2Desc: "Mercedes esclusivi, sanificati e con manutenzione certificata.",
      standard3Title: "Autisti Qualificati",
      standard3Desc: "Professionisti multilingue con formazione corporate.",
      standard4Title: "Fatturazione Aziendale",
      standard4Desc: "Processo semplificato con fattura elettronica e reportistica dettagliata.",
      flexibilityTitle: "Flessibilità Operativa",
      flexibilityText: "Adattiamo i nostri servizi alle esigenze specifiche di ogni cliente corporate. Dalle disposizioni orarie ai contratti a lungo termine, offriamo soluzioni personalizzate con condizioni dedicate.",
      ctaContact: "Contatto B2B",
      ctaWhatsApp: "WhatsApp Business",
      ctaDiscover: "Scopri l'Eccellenza",
      tariffsTitle: "Prontuario Tariffe Corporate",
      tariffsSubtitle: "Listino Pubblico e Ufficiale",
      tariffsDescription: "Prezzi chiari e trasparenti per servizi corporate. Tutte le tariffe sono IVA esclusa.",
      hourlyTitle: "Disposizioni Orarie",
      hours4: "4 ore",
      hours8: "8 ore",
      hours4Price: "€360",
      hours8Price: "€680",
      transfersTitle: "Transfer Aeroportuali",
      malpensaPrice: "€170",
      linatePrice: "€100",
      bergamoPrice: "€180",
      kmTitle: "Tariffa Chilometrica",
      kmBase: "€1,50/km",
      kmMountain: "€1,95/km (+30% laghi/montagna)",
      notes: "* Tariffe per Mercedes Classe V (fino a 6 pax). Per Classe S o veicoli speciali, richiedere preventivo dedicato.",
      invoiceNote: "Fatturazione elettronica disponibile. Condizioni speciali per contratti annuali.",
    },
    aboutNexitus: {
      tagline: "CHI SIAMO — NEXITUS",
      title: "NEXITUS",
      subtitle: "Executive Mobility & Chauffeur Services",
      description: "NEXITUS è il punto di riferimento per la mobilità esecutiva premium nel Nord Italia e in tutta Europa. Specializzati in transfer aeroportuali per Malpensa, Linate e Orio al Serio, servizi a disposizione oraria e logistica completa per eventi aziendali e viaggi d'affari, offriamo un'esperienza dove l'eccellenza è l'unico standard accettato.",
      pillar1Title: "Flotta Mercedes-Benz Esclusiva",
      pillar1Desc: "Veicoli nuovi, impeccabilmente puliti e sanificati dopo ogni viaggio. Comfort e raffinatezza in ogni dettaglio.",
      pillar2Title: "Autisti Professionisti Multilingue",
      pillar2Desc: "Puntualità assoluta, discrezione totale e accoglienza con cartello nominativo all'arrivo. Professionisti formati secondo standard corporate.",
      pillar3Title: "Trasparenza e Flessibilità B2B",
      pillar3Desc: "Prezzi fissi concordati in anticipo, fatturazione elettronica aziendale e assistenza dedicata 24 ore su 24, 7 giorni su 7.",
      ctaServices: "Scopri i Nostri Servizi",
      ctaContact: "Contatto B2B",
    },
  },
  fr: {
    header: {
      brand: "Nexus Company",
      inicio: "Accueil",
      chiSiamo: "Qui sommes-nous?",
      nostriVeicoli: "Nos véhicules",
      servicos: "Services",
      contato: "Contact",
      reserveAgora: "Réserver",
    },
    common: {
      whatsappMessage: "Bonjour, je viens du site Nexus",
    },
    hero: {
      title: "RÉSERVEZ VOTRE VOITURE MAINTENANT",
      titleHighlight: "",
      subtitle:
        "Nexus est la plateforme de mobilité premium conçue pour tous vos besoins de transfert et d'événements, disponible dans plusieurs villes.",
      cta: "Demander une course",
      ctaContact: "Contactez-nous",
    },
    chiSiamo: {
      title: "Qui sommes-nous?",
      subtitle: "Professionnalisme, fiabilité et confort",
      description:
        "Nexus est votre service NCC (Noleggio Con Conducente) de référence à Milan. Nous proposons des transferts aéroportuaires vers et depuis Malpensa, Linate et Bergamo Orio al Serio avec des véhicules Mercedes de luxe et des chauffeurs professionnels multilingues. Contrairement aux taxis traditionnels, nous garantissons un prix fixe communiqué à l'avance, sans surprises, sans suppléments.",
      descriptionExtra:
        "Nous opérons 24 heures sur 24, 7 jours sur 7 avec surveillance des vols en temps réel. Notre chauffeur vous attend à l'aéroport avec une pancarte nominative, vous aide avec les bagages et vous accompagne jusqu'au véhicule climatisé.",
    },
    experienceGallery: {
      title: "Nos Expériences",
      description: "Découvrez des destinations incroyables et la qualité de nos services",
    },
    whyChoose: {
      subtitle: "MODÈLE OPÉRATIF D'EXCELLENCE",
      title: "LE STANDARD NEXITUS DE MOBILITÉ",
      description:
        "Notre histoire est née d'années de dévouement auprès d'une clientèle d'élite. Nous avons transformé cette expérience en un modèle opératif où la perfection est la norme et la réactivité anticipe chaque exigence.",
      feature1: "LA PERFECTION COMME NORME",
      feature2: "RÉACTIVITÉ & ANTICIPATION",
      feature3: "CLIENTÈLE D'ÉLITE & DISCRÉTION",
      cardTitle: "Modèle Opératif & Engagement Qualité NEXITUS",
      cardItem1Title: "✓ Précision & Ponctualité Absolue",
      cardItem1Desc: "Planification préventive et suivi des vols en temps réel pour zéro tolérance au retard.",
      cardItem2Title: "✓ Flotte Mercedes-Benz d'Élite",
      cardItem2Desc: "Véhicules désinfectés, confort exécutif, Wi-Fi à bord et isolation acoustique optimale.",
      cardItem3Title: "✓ Chauffeurs Dédiés & Accueil VIP",
      cardItem3Desc: "Service Meet & Greet avec pancarte nominative, aide aux bagages et protocole discret.",
      cardItem4Title: "✓ Support Opérationnel B2B 24/7",
      cardItem4Desc: "Disponibilité continue pour transferts aéroportuaires, mises à disposition et événements.",
    },
    nostriVeicoli: {
      title: "Nos véhicules",
      subtitle: "Flotte Mercedes de luxe pour votre confort",
      vehicle1Title: "Mercedes Benz v250",
      vehicle1Desc:
        "Berline élégante et confortable, parfaite pour les transferts et voyages d'affaires. Capacité: jusqu'à 3 passagers + bagages. Équipée de climatisation, sièges en cuir et technologie de dernière génération.",
      vehicle2Title: "Mercedes Classe V",
      vehicle2Desc:
        "Van de luxe idéal pour les groupes et familles. Capacité: jusqu'à 7 passagers + bagages. Espace spacieux, confort supérieur et parfait pour les tours touristiques et transferts aéroportuaires avec plusieurs personnes.",
      calcPreventivo: "Calculer le devis",
      passengers: "passagers",
      luggage: "Bagages",
    },
    services: {
      title: "Nos Services",
      subtitle: "Services de noleggio con conducente pour tous vos besoins de transport exécutif",
      touristTitle: "Tours Touristiques",
      touristDesc: "Lacs et destinations avec chauffeur privé",
      disposalTitle: "Mise à Disposition",
      disposalDesc: "Voiture à disposition par heure - 2h, 4h ou 8h",
      whatsappTitle: "Réserver WhatsApp 24h",
      whatsappDesc: "Service immédiat via WhatsApp",
      transfersTitle: "Transferts Aéroports",
      transfersDesc: "Malpensa, Linate, Bergamo porte-à-porte",
      longDistanceTitle: "Longue Distance",
      longDistanceDesc: "Tarifs au km pour toute destination",
    },
    touristTours: {
      title: "Tours Touristiques",
      subtitle: "Découvrez les plus beaux lacs et destinations de la région avec confort et style",
      lagoComoTitle: "Lac de Côme",
      lagoComoDesc: "Tour complet du magnifique Lac de Côme",
      lagoMaggioreTitle: "Lac Majeur",
      lagoMaggioreDesc: "Explorez les îles et jardins du Lac Majeur",
      sanSiroTitle: "Stade San Siro",
      sanSiroDesc: "Visitez le temple du football italien",
      priceFrom: "À partir de",
      duration: "Durée",
      hours: "heures",
      bookNow: "Réserver",
    },
    hourlyDisposal: {
      title: "Services de Noleggio con Conducente",
      subtitle: "Mise à Disposition Horaire",
      description: "Voiture à disposition avec chauffeur privé pour vos besoins professionnels ou personnels",
      package2h: "Forfait 2 Heures",
      package4h: "Forfait 4 Heures",
      package8h: "Forfait 8 Heures",
      price: "Prix",
      features: "Inclus",
      professionalDriver: "Chauffeur professionnel",
      luxuryCar: "Véhicule Mercedes de luxe",
      flexibility: "Flexibilité totale d'itinéraire",
      assistance: "Assistance 24h",
      bookNow: "Demander un devis",
    },
    transfers: {
      title: "Transferts Aéroports",
      subtitle: "Service porte-à-porte vers tous les aéroports de Milan",
      mainTitle: "Nous proposons un service de location avec chauffeur vers et depuis l'aéroport de Milan Malpensa.",
      chooseService: "Choisissez notre service et profitez de plusieurs avantages:",
      flightMonitoring: "planifiez votre voyage à l'avance en utilisant l'un des véhicules à votre disposition",
      comfortTravel: "voyagez en tout confort dans une voiture de luxe",
      inclusivePrice:
        "connaissez le prix inclusif de votre trajet à l'avance: il est fourni lorsque vous demandez un devis, afin d'éviter les mauvaises surprises et les suppléments coûteux",
      doorToDoor:
        "atteignez rapidement votre destination finale: un service porte-à-porte qui vous fera gagner du temps et vous garantira une expérience exceptionnelle",
      highlightTitle: "VOTRE TRANSFERT VERS ET DEPUIS MILAN MALPENSA, SANS RISQUE ET SANS STRESS",
      contactUs: "CONTACTEZ-NOUS",
      malpensa: "Aéroport Malpensa",
      linate: "Aéroport Linate",
      bergamo: "Bergamo/Orio al Serio",
      priceFrom: "À partir de",
      features: "Service Inclus",
      meetGreet: "Accueil personnalisé",
      luggageAssistance: "Assistance bagages",
      professionalDriver: "Chauffeur professionnel",
      bookNow: "Réserver un transfert",
      pricesTitle: "Prix des Transferts Aéroports",
      malpensaMilano: "Malpensa – Milan",
      linateMilano: "Linate – Milan",
      orioMilano: "Orio al Serio – Milan",
    },
    longDistance: {
      title: "Longue Distance - Tarifs au Km",
      subtitle: "Services de Noleggio con Conducente vers toute destination en Italie",
      description: "Voyagez confortablement vers n'importe quelle ville italienne",
      perKm: "par km",
      examples: "Exemples de Trajets",
      milanoBari: "Milano → Bari",
      milanoNapoli: "Milano → Napoli",
      milanoRoma: "Milano → Roma",
      milanoFirenze: "Milano → Firenze",
      anyDestination: "Milano → N'importe quelle adresse",
      features: "Inclus dans le Service",
      professionalDriver: "Chauffeur expérimenté",
      luxuryCar: "Mercedes Classe E ou V",
      stops: "Arrêts autorisés",
      assistance: "Assistance 24h",
      bookNow: "Demander un devis",
    },
    locations: {
      title: "Explorez Milan en Exclusivité",
      subtitle:
        "Découvrez les principales attractions touristiques de la ville avec le confort et l'élégance que vous méritez",
      historicTitle: "San Siro",
      historicDesc:
        "Visitez le légendaire stade San Siro, domicile des géants du football italien Inter et Milan, un temple du sport mondial",
      luxuryTitle: "Lac Majeur",
      luxuryDesc:
        "Découvrez les eaux cristallines et les îles enchanteresses du Lac Majeur, avec ses jardins historiques et ses vues à couper le souffle",
      cultureTitle: "Lac de Côme",
      cultureDesc:
        "Explorez l'élégant Lac de Côme, entouré de villas luxueuses, de montagnes majestueuses et de villages pittoresques",
    },
    finalCta: {
      title: "L'excellence a une nouvelle adresse.",
      titleHighlight: "Bienvenue dans le monde NEXITUS.",
      subtitle: "Connectez-vous à notre plateforme pour suivre l'évolution du service haut de gamme.",
      subtitleExtra: "Perfection opérationnelle, discrétion et réactivité au service de la mobilité d'élite.",
      cta: "Demander le Service Exécutif",
    },
    tariffs: {
      title: "Calculer le devis",
      description:
        "Consultez nos tarifs pour les destinations de montagne, lacs, plages et grandes villes, au départ de l'Aéroport de Malpensa.",
      viewAll: "Voir tous les tarifs",
      pageTitle: "Calculer le devis",
      pageSubtitle: "Au départ de l'Aéroport de Malpensa",
      distanceNote: "Toutes les distances indiquées sont aller-retour (A/R).",
      mountainsTitle: "MONTAGNES & LACS (+30%)",
      seaTitle: "MER & GRANDES VILLES",
      kmLabel: "Km (A/R):",
      priceLabel: "Prix:",
      ratesBox: {
        baseRate: "Tarif kilométrique de base:",
        mountainRate: "Lacs et Montagnes:",
        hourlyRate: "Mise à disposition horaire:",
        olympicsRate: "Jeux Olympiques Milano-Cortina:",
      },
      bookNow: "Réserver",
      milanoTransferTitle: "Milan - Transfer Simple",
      corporateTitle: "Corporate (avec facture)",
      corporateHours4: "4 heures:",
      corporateHours8: "8 heures:",
      capacityTitle: "Capacité",
      capacityDesc: "Jusqu'à 5–6 passagers. Pour confirmer le nombre de bagages, contactez-nous via WhatsApp.",
    },
    footer: {
      brand: "Nexus, the living experience",
      brandDesc:
        "Votre chauffeur privé de confiance à Milan. Transport de luxe avec excellence, confort et ponctualité.",
      contactTitle: "Contact",
      whatsapp: "WhatsApp",
      socialTitle: "Réseaux Sociaux",
      copyright: "© 2025 Nexus. Tous droits réservés.",
    },
    contact: {
      needInfo: "BESOIN DE PLUS D'INFORMATIONS?",
      title: "CONTACTEZ-NOUS",
      welcomeText: "Nous sommes là pour vous aider à planifier votre voyage rapidement et facilement.",
      form: {
        name: "Nom",
        email: "E-mail",
        phone: "Téléphone",
        message: "Message",
        privacyConsent:
          "Je consens au traitement de mes données personnelles conformément aux procédures et limitations énoncées dans la déclaration de confidentialité.",
        privacyLink: "Lire la Déclaration de Confidentialité",
        send: "Envoyer",
        namePlaceholder: "Votre nom",
        emailPlaceholder: "votre@email.com",
        messagePlaceholder: "Comment pouvons-nous vous aider?",
      },
      info: {
        phone: "Téléphone",
        phoneNumber: "+39 389 143 0907",
        whatsapp: "WhatsApp",
        whatsappNumber: "+39 389 143 0907",
        email: "E-mail",
        emailAddress: "thomastagliatti2022@gmail.com",
        office: "Siège Social",
        officeAddress: "Milano, Italie",
      },
      success: "Message envoyé avec succès!",
      error: "Erreur lors de l'envoi du message. Réessayez.",
    },
    milano: {
      navLabel: "Nexus Company",
      heroTitle: "MILANO",
      heroSubtitle: "Premium Mobility",
      heroTagline: "by Nexus",
      heroDescription: "Division corporate dédiée aux clients B2B. Mobilité executive premium avec standards internationaux d'excellence.",
      manifestoTitle: "Manifeste Opératif",
      manifestoText: "MILANO représente l'évolution naturelle de l'excellence Nexus pour le marché corporate. Notre approche opérative combine expérience consolidée avec flexibilité totale, garantissant des solutions de mobilité sur mesure pour chaque besoin d'entreprise.",
      standardsTitle: "Standards de Service",
      standard1Title: "Ponctualité Absolue",
      standard1Desc: "Monitoring des vols en temps réel. Zéro tolérance aux retards.",
      standard2Title: "Flotte Premium",
      standard2Desc: "Mercedes exclusifs, désinfectés et avec maintenance certifiée.",
      standard3Title: "Chauffeurs Qualifiés",
      standard3Desc: "Professionnels multilingues avec formation corporate.",
      standard4Title: "Facturation d'Entreprise",
      standard4Desc: "Processus simplifié avec facture électronique et rapports détaillés.",
      flexibilityTitle: "Flexibilité Opérative",
      flexibilityText: "Nous adaptons nos services aux exigences spécifiques de chaque client corporate. Des mises à disposition horaires aux contrats à long terme, nous offrons des solutions personnalisées avec conditions dédiées.",
      ctaContact: "Contact B2B",
      ctaWhatsApp: "WhatsApp Business",
      ctaDiscover: "Découvrir l'Excellence",
      tariffsTitle: "Tarifs Corporate",
      tariffsSubtitle: "Liste de Prix Officielle",
      tariffsDescription: "Prix clairs et transparents pour services corporate. Tous les tarifs sont HT.",
      hourlyTitle: "Mise à Disposition Horaire",
      hours4: "4 heures",
      hours8: "8 heures",
      hours4Price: "€360",
      hours8Price: "€680",
      transfersTitle: "Transferts Aéroports",
      malpensaPrice: "€170",
      linatePrice: "€100",
      bergamoPrice: "€180",
      kmTitle: "Tarif Kilométrique",
      kmBase: "€1,50/km",
      kmMountain: "€1,95/km (+30% lacs/montagne)",
      notes: "* Tarifs pour Mercedes Classe V (jusqu'à 6 pax). Pour Classe S ou véhicules spéciaux, demander devis dédié.",
      invoiceNote: "Facturation électronique disponible. Conditions spéciales pour contrats annuels.",
    },
    aboutNexitus: {
      tagline: "À PROPOS DE NEXITUS",
      title: "NEXITUS",
      subtitle: "Executive Mobility & Chauffeur Services",
      description: "NEXITUS est la référence en mobilité exécutive premium dans le Nord de l'Italie et à travers toute l'Europe. Spécialisés dans les transferts aéroportuaires vers Malpensa, Linate et Orio al Serio, les services de mise à disposition horaire et la logistique complète pour événements d'entreprise et voyages d'affaires, nous offrons une expérience où l'excellence est le seul standard accepté.",
      pillar1Title: "Flotte Mercedes-Benz Exclusive",
      pillar1Desc: "Véhicules neufs, impeccablement propres et désinfectés après chaque trajet. Confort et raffinement dans chaque détail.",
      pillar2Title: "Chauffeurs Professionnels Multilingues",
      pillar2Desc: "Ponctualité absolue, discrétion totale et accueil personnalisé avec pancarte nominative à l'arrivée. Professionnels formés aux standards corporate.",
      pillar3Title: "Transparence et Flexibilité B2B",
      pillar3Desc: "Prix fixes convenus à l'avance, facturation électronique d'entreprise et assistance dédiée 24 heures sur 24, 7 jours sur 7.",
      ctaServices: "Découvrir Nos Services",
      ctaContact: "Contact B2B",
    },
  },
};

export type TranslationKey = keyof typeof translations.pt;
